    ...
    for P_control in P_list:
        ...
        for ic in initial_conditions:
            ...
            if not any(np.linalg.norm(refined - np.array(prev)) < 1e-3 for prev in found):
                ...
                s1_Re.append(s1.real)
                s2_Re.append(s2.real)
                stabilities.append(is_stable)
    return P_vals, s1_abs2_list, s2_abs2_list, stabilities,s1_Re,s2_Re
...
start=time.time()
P_up, s1_up, s2_up, stable_up,s1_Re_up,s2_Re_up = bidirectional_sweep(P_control_range)
...
s1_Re_stable = [s for s, st in zip(s1_Re_up, stable_up) if st]

P_unstable = [P for P, st in zip(P_up, stable_up) if not st]
s1_unstable = [s for s, st in zip(s1_up, stable_up) if not st]
s1_Re_unstable = [s for s, st in zip(s1_Re_up, stable_up) if not st]

# Create the plot
fig, ax = plt.subplots(figsize=(8, 6))

# Plot stable branches
ax.plot(P_stable, s1_stable, 'o', color='blue', label=r'Stable $|s_1|^2$', markersize=4)
ax.plot(P_stable, s1_Re_stable, 'o', color='black', label=r'Stable Re{$s_1$}', markersize=4)
...
# Labels and title
ax.set_xlabel(r'$P_{\mathrm{control}}$ (W)')
ax.set_ylabel(r'$|s_1|^2$ and Re{$s_1$}')
ax.set_title('Bifurcation Diagram of $s_1$')
...
# Save the figure
plt.savefig('bifurcation_diagram_s1_full.png', dpi=300)
...
with open('bifurcation_data.csv', mode='w', newline='') as file:
    ...
    # Write each row
    for P, s1, s2, st in zip(P_up, s1_up, s2_up, stable_up):
        writer.writerow([
            P,
