...
% % calculate T0
% % %mm=tao_theta.*xi_T.*abs(Ys(:,1))^2.*(eta_lin.*eta_c+2.*alpha_TPA.*abs(Ys(:,1))^2+2.*gamma_FCA.*tao.*abs(Ys(:,1))^4)
...
%% plot the development of a_real and a_imag 
% tSpan=[0 30];
% [T,Y]=ode45(@(t,y) fun(t,y), tSpan, [0.4+0.4i,0.5,1.5]);
% figure(1)
% plot(T,real(Y(:,1)));
% hold on
% plot(T,imag(Y(:,1)))
% xlabel('t')
% legend('a\_real','a\_imag','Location','best')
% figure(2)
...
% 
% tSpan=[0 30];
% [T,Y]=ode45(@(t,y) funN(t,y), tSpan, [0.4+0.4i,0.6]);
% figure(1)
% plot(T,real(Y(:,1)));
% hold on
% plot(T,imag(Y(:,1)));
...
% plot(T,real(Y(:,2)));
% xlabel('t')
% legend('a\_real','a\_imag','|a|','n','Location','best')
% figure(2)
% plot(real(Y(:,1)),imag(Y(:,1)))
% xlabel('a\_real')
% ylabel('a\_imag')
...
tSpan=[0 20];
[T,Y]=ode45(@(t,y) funT(t,y), tSpan, [0.4+0.4i,1.5]);
figure(1)
plot(T,real(Y(:,1)));
hold on
plot(T,imag(Y(:,1)));
...
plot(T,Y(:,2));
xlabel('t')
legend('a\_real','a\_imag','|a|','T','Location','best')
figure(2)
plot(real(Y(:,1)),imag(Y(:,1)))
xlabel('a\_real')
ylabel('a\_imag')
